...
// ADD after imports — decompress SVG body based on X-SVG-Encoding header
function decompressSvgBody(buf, req) {
  const encoding = (
  ...
  try {
    ...
    console.warn(
      `[decompress] Failed to decompress ${encoding}: ${e.message} — using raw`,
    );
  }
  return buf;
}
...
async function renderSvg(svgText, format) {
  return pool.render(
    await embedExternalImages(svgText, "SpicyDevs-Rasterizer/3.1"),
    format,
  );
}
