...
// Apply identical changes to node/index.js
...
// __dir = this file's directory = render-node/  (or node/)
// This is passed to RenderPool as serverDir so workers can resolve
// @resvg/resvg-js from render-node/node_modules/ via createRequire.
...
async function buildResvgOpts() {
  ...
  return {
    fitTo: { mode: 'original' }, imageRendering: 0,
    font: {
      loadSystemFonts: false, ...result,
...
// ── Body reader ───────────────────────────────────────────────────────────────
// Tags client-abort errors (_clientAbort=true) so the outer catch can stay
// silent — these happen normally when the CF Worker's AbortController fires
// because another rasterizer backend won the race.
...
async function fetchFromWsrv(svgUrl, format) {
  ...
  try {
    const res = await fetch(url.toString(), { signal: ac.signal, headers: { 'User-Agent': 'SpicyDevs-Rasterizer/2.0' } });
    clearTimeout(t);
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/health') {
    ...
    return res.end(JSON.stringify({
      status: 'ok', version: '2.1', ts: Date.now(),
      ...
      serverDir:       __dir,          // visible in /health for debugging
  ...
  try {
    ...
    if (req.headers['content-type'] === 'application/json') {
      ...
      if (!Array.isArray(payload.jobs)) { res.writeHead(400, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: "Expected 'jobs' array" })); }
      ...
      const results = await Promise.all(payload.jobs.map(async job => {
        ...
        try {
          ...
          await logError('Bulk job failed (no fallback URL)', resvgErr.message, [{ name: 'Job ID', value: String(job.id), inline: true }]);
    ...
    if (req.method === 'POST') {
      ...
      if (!targetSvgUrl) { res.writeHead(400, { 'Content-Type': 'text/plain' }); return res.end('Missing ?url= parameter'); }
      const svgRes = await fetch(targetSvgUrl, { headers: { 'User-Agent': 'SpicyDevs-Rasterizer/2.0' }, signal: AbortSignal.timeout(8_000) });
      if (!svgRes.ok) { res.writeHead(502, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: `SVG fetch failed: ${svgRes.status}` })); }
    ...
    try {
      ...
      if (fallbackUrl) {
        ...
        await logError('Single job failed (no fallback URL)', resvgErr.message, [{ name: 'Format', value: format, inline: true }]);
    ...
    // Client abort: CF Worker dropped connection because another backend won.
    // Expected — do NOT log to Discord or count as error.
...
(async () => {
  ...
  console.log('[pool] serverDir:', __dir);
  console.log('[pool] workerPath:', WORKER_PATH);

  // Pass __dir so workers can resolve @resvg/resvg-js from render-node/node_modules/
