...
// FONT STRATEGY
// ─────────────
// All platforms ship JakartaSans-Regular.ttf (Plus Jakarta Sans Regular).
...
// JakartaSans-Regular.ttf is copied alongside it so the path.join(_moduleDir, …)
...
const FONT_SRC = (() => {
  const p = path.join(_moduleDir, "JakartaSans-Regular.ttf");
  try {
    fs.accessSync(p, fs.constants.R_OK);
    return p;
  } catch {
    console.error(
      "[sharedRender] JakartaSans-Regular.ttf not found next to sharedRender.js",
    );
    return null;
  }
})();
...
export function getFontPath() {
  ...
  const dst = path.join(os.tmpdir(), "JakartaSans-Regular.ttf");
  try {
    ...
    _tmpFontPath = dst;
    console.log(`[font] JakartaSans-Regular staged → ${dst}`);
  } catch (e) {
