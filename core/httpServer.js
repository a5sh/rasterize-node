...
export function decompressSvgBody(buf, req) {
  ...
  try {
    ...
    if (encoding === 'br' || encoding === 'brotli') return brotliDecompressSync(buf);
  } catch (e) {
    console.warn(`[decompress] Failed (${encoding}): ${e.message} — using raw body`);
  }
  return buf;
}
...
export function createRasterServer({
  ...
  async function handleJsonBody(req, res, params, bodyBuf) {
    ...
    let payload;
    try { payload = JSON.parse(bodyBuf); } catch {
      res.writeHead(400, { 'Content-Type': 'application/json' });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.headers['content-type']?.includes('application/json')) return handleJsonBody(req, res, params, bodyBuf);
